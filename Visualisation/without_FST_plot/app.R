...
message_fst <- ""
save_fst_title <- ""
if (u_parameters[1,21] == "chr_no") {
  message_fst <- "<p style='color:#3EA055;font-weight:bold'>The Manhattan plot cannot be created since there are no chromosomes with variants more than or equal to 100 and less or equal to 100,000.</p>"
  save_fst_title <- "Save selected data as a zip file"
} else if (u_parameters[1,21] == "gr_no") {
  if (u_parameters[1,11] == "yes") {
    message_fst <- "<p style='color:#3EA055;font-weight:bold'>The number of clusters is large. Reduce the number of clusters (less than or equal to 5) to calculate the per-variant FST estimates for each cluster pair and visualize the results as a Manhattan plot.</p>"
  } else if (u_parameters[1,11] == "no") {
    message_fst <- "<p style='color:#3EA055;font-weight:bold'>The number of groups is large. Reduce the number of groups (less than or equal to 5) to calculate the per-variant FST estimates for each group pair and visualize the results as a Manhattan plot.</p>"
  }
  save_fst_title <- "Save FST summary report as a zip file"
}

cultivar_fn_orig <- ""
cultivar_fn <- ""
if (u_parameters[1,13] == 1) {
  cultivar_fn_orig <- "data/cultivars_orig.list"
  cultivar_fn <- "data/cultivars.list"
} else if (u_parameters[1,13] == 0) {
  cultivar_fn_orig <- "data/plink_samples_info_orig.fam"
  cultivar_fn <- "data/plink_samples_info.fam"
}
cultivar_orig <- read.table(file = cultivar_fn_orig, header = FALSE)
cult_orig_number <- nrow(cultivar_orig)
cultivar <- read.table(file = cultivar_fn, header = FALSE)
cultivars_number <- nrow(cultivar)
...
het_matrix <- matrix(, nrow = cultivars_number, ncol = 7)
ibc_matrix <- matrix(, nrow = cultivars_number, ncol = 6)
...
if (u_parameters[1,13] == 1) {
  cultivar_ibs_fn <- "data/results_data.mibs.cultivars.list"
  cultivar_ibs_os_fn <- "data/os_results_data.mibs.cultivars.list"
  cultivar_rel_fn <- "data/results_data.rel.cultivars.list"
  cultivar_rel_os_fn <- "data/os_results_data.rel.cultivars.list"
  cultivar_king_fn <- "data/results_data.king.cultivars.list"
  cultivar_king_os_fn <- "data/os_results_data.king.cultivars.list"
} else if (u_parameters[1,13] == 0) {
  cultivar_ibs_fn <- "data/results_data.mibs.id"
  cultivar_ibs_os_fn <- "data/os_results_data.mibs.id"
  cultivar_rel_fn <- "data/results_data.rel.id"
  cultivar_rel_os_fn <- "data/os_results_data.rel.id"
  cultivar_king_fn <- "data/results_data.king.id"
  cultivar_king_os_fn <- "data/os_results_data.king.id"
}
 
cultivar_ibs <- read.table(file = cultivar_ibs_fn, header = TRUE)
cultivar_ibs_os <- read.table(file = cultivar_ibs_os_fn, header = TRUE)
cultivar_rel <- read.table(file = cultivar_rel_fn, header = TRUE)
cultivar_rel_os <- read.table(file = cultivar_rel_os_fn, header = TRUE)
cultivar_king <- read.table(file = cultivar_king_fn, header = TRUE)
cultivar_king_os <- read.table(file = cultivar_king_os_fn, header = TRUE)

rownames(ibs_matrix) <- cultivar_ibs[,2]
colnames(ibs_matrix) <- cultivar_ibs[,2]
rownames(rel_matrix) <- cultivar_rel[,2]
colnames(rel_matrix) <- cultivar_rel[,2]
rownames(king_matrix) <- cultivar_king[,2]
colnames(king_matrix) <- cultivar_king[,2]

rownames(ibs_matrix_os) <- cultivar_ibs_os[,2]
colnames(ibs_matrix_os) <- cultivar_ibs_os[,2]
rownames(rel_matrix_os) <- cultivar_rel_os[,2]
colnames(rel_matrix_os) <- cultivar_rel_os[,2]
rownames(king_matrix_os) <- cultivar_king_os[,2]
colnames(king_matrix_os) <- cultivar_king_os[,2]
  ...
  tabsetPanel(
    tabPanel(
      ...
      conditionalPanel(condition = "input.output_type=='Chart'",
        conditionalPanel(condition = "(input.dataset_type=='Original' && input.bs_reports_orig=='Sample variant-count report') || (input.dataset_type=='After filtering' && input.bs_reports=='Sample variant-count report')",
          plotlyOutput("view_vc", height = "600px", inline = FALSE)
        ),
        conditionalPanel(condition = "(input.dataset_type=='Original' && input.bs_reports_orig=='Sample-based missing data report') || (input.dataset_type=='After filtering' && input.bs_reports=='Sample-based missing data report')",
    ...
    tabPanel(
      ...
      conditionalPanel(condition = "input.data_type_pca=='PCA'",
        ...
        tags$span(id = "comment_text", a(href = "https://www.cog-genomics.org/plink/2.0/strat#pca", target = "_blank", "--pca"), " extracts the top 10 principal components of the variance-standardized relationship matrix computed by PLINK 2.0 ", a(href = "https://www.cog-genomics.org/plink/2.0/distance#make_rel", target = "_blank", "--make-rel"), "."),
        tags$br(),
      ...
      selectInput(inputId = "cultivars_pca", label = "List of Sample ID/Name:", choices = c("ALL",  sort(as.character(cultivar[, 2])))),
      radioButtons("display_sam_name", "Samples IDs/Names:", choices = c("Hide", "Display"), inline = T),
      plotlyOutput("view_pca", width = "1000px", height = "800px", inline = FALSE),
      tags$br(),
      fluidRow(
    ...
    tabPanel(
      ...
      tags$br(),
      tags$span(id = "comment_text", "which elaborates on Hudson RR, Slatkin M, Maddison WP (1992) \"Estimation of Levels of Gene Flow from DNA Sequence Data\"."),
      tags$br(),
      tags$br(),
      ...
      tags$br(),
      tags$br(),
      downloadButton("save_text_fst", save_fst_title),
      tags$br(),
      tags$br()
    ),
    tabPanel(
      ...
      conditionalPanel(condition = "input.data_type_ksh=='Relationship matrix'",
        tags$span(id = "param_title", "Variance-standardized genomic relationship matrix"),
        tags$span(id = "comment_text", "(Genomic Relationship Matrix, GRM)"),
...
# Basic statistics
  ...
  output$variants_number <- renderText({
    if (input$dataset_type == "Original") {
      ...
      if (input$bs_reports == "Sample variant-count report" || input$bs_reports == "Sample-based missing data report") {
        sprintf("Number of variants used in analysis (remained after filtering): %s; Number of analyzed samples: %s", u_parameters[1,16], cultivars_number)
      } else if (input$bs_reports == "Method-of-moments F coefficient estimates" || input$bs_reports == "GCTA inbreeding coefficient report") {
        sprintf("Number of variants used in analysis (remained after filtering and LD pruning): %s; Number of analyzed samples: %s", u_parameters[1,17], cultivars_number)
      }
    }
  })
  ...
  bs_data <- reactive({
    if (input$dataset_type == "Original") {
      if (input$bs_reports_orig == "Sample variant-count report") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar_orig[,2]), HOM_REF_CT  = as.numeric(sample_counts_matrix[,2]), HOM_ALT_SNP_CT = as.numeric(sample_counts_matrix[,3]), HET_SNP_CT = as.numeric(sample_counts_matrix[,4]), DIPLOID_TRANSITION_CT = as.numeric(sample_counts_matrix[,5]), DIPLOID_TRANSVERSION_CT = as.numeric(sample_counts_matrix[,6]), DIPLOID_NONSNP_NONSYMBOLIC_CT = as.numeric(sample_counts_matrix[,7]), DIPLOID_SINGLETON_C = as.numeric(sample_counts_matrix[,8]), HAP_REF_CT = as.numeric(sample_counts_matrix[,9]), HAP_ALT_CT = as.numeric(sample_counts_matrix[,10]), MISSING_CT = as.numeric(sample_counts_matrix[,11])) %>% dplyr::arrange(SAMPLE_ID)
      } else if (input$bs_reports_orig == "Sample-based missing data report") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar_orig[,2]), MISSING_CT  = as.numeric(missing_data_matrix[,2]), OBS_CT = as.numeric(missing_data_matrix[,3]), F_MISS = as.numeric(missing_data_matrix[,4])) %>% dplyr::arrange(SAMPLE_ID)
      }
    } else if (input$dataset_type == "After filtering") {
      if (input$bs_reports == "Sample variant-count report") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar[,2]), HOM_REF_CT  = as.numeric(sample_counts_af_matrix[,2]), HOM_ALT_SNP_CT = as.numeric(sample_counts_af_matrix[,3]), HET_SNP_CT = as.numeric(sample_counts_af_matrix[,4]), DIPLOID_TRANSITION_CT = as.numeric(sample_counts_af_matrix[,5]), DIPLOID_TRANSVERSION_CT = as.numeric(sample_counts_af_matrix[,6]), DIPLOID_NONSNP_NONSYMBOLIC_CT = as.numeric(sample_counts_af_matrix[,7]), DIPLOID_SINGLETON_C = as.numeric(sample_counts_af_matrix[,8]), HAP_REF_CT = as.numeric(sample_counts_af_matrix[,9]), HAP_ALT_CT = as.numeric(sample_counts_af_matrix[,10]), MISSING_CT = as.numeric(sample_counts_af_matrix[,11])) %>% dplyr::arrange(SAMPLE_ID)
      } else if (input$bs_reports == "Sample-based missing data report") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar[,2]), MISSING_CT  = as.numeric(missing_data_af_matrix[,2]), OBS_CT = as.numeric(missing_data_af_matrix[,3]), F_MISS = as.numeric(missing_data_af_matrix[,4])) %>% dplyr::arrange(SAMPLE_ID)
      } else if (input$bs_reports == "Method-of-moments F coefficient estimates") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar[,2]), O_HOM  = as.numeric(het_matrix[,2]), E_HOM = as.numeric(het_matrix[,3]), O_HET  = as.numeric(het_matrix[,4]), E_HET  = as.numeric(het_matrix[,5]), OBS_CT = as.numeric(het_matrix[,6]), F = as.numeric(het_matrix[,7])) %>% dplyr::arrange(SAMPLE_ID)
      } else if (input$bs_reports == "GCTA inbreeding coefficient report") {
        dt_bs <- data.frame(SAMPLE_ID = as.character(cultivar[,2]), NOMISS  = as.numeric(ibc_matrix[,3]), Fhat1 = as.numeric(ibc_matrix[,4]), Fhat2 = as.numeric(ibc_matrix[,5]), Fhat3 = as.numeric(ibc_matrix[,6])) %>% dplyr::arrange(SAMPLE_ID)
      }
    }
  })
  ...
  dataset_input_vc <- reactive({
    if ((input$dataset_type == "Original" && input$bs_reports_orig == "Sample variant-count report") || (input$dataset_type == "After filtering" && input$bs_reports == "Sample variant-count report")) {
      ...
      layout(barmode = "stack", paper_bgcolor = "#FFE4E1", plot_bgcolor = "#FFFFFF",
        title = list(text = "Sample variant-count report", font = list(family = "sans serif", size = 20, color = "#C71585")),
        font = list(family = "sans serif", size = 14, color = "#4B0082"), margin = 1,
        xaxis = list(title = "Sample ID", ticklen = 5), yaxis = list(title = "Count", ticklen = 5)) %>%
  ...
  dataset_input_miss <- reactive({
    if ((input$dataset_type == "Original" && input$bs_reports_orig == "Sample-based missing data report") || (input$dataset_type == "After filtering" && input$bs_reports == "Sample-based missing data report")) {
      ...
      layout(paper_bgcolor = "#FFE4E1", plot_bgcolor = "#FFFFFF", title = list(text = "Sample-based missing data report", font = list(family = "sans serif", size = 20, color = "#C71585")),
        font = list(family = "sans serif", size = 14, color = "#4B0082"), margin = 1, xaxis = list(title = "Sample ID", ticklen = 5)) %>%
  ...
  dataset_input_het <- reactive({
    if (input$dataset_type == "After filtering" && input$bs_reports == "Method-of-moments F coefficient estimates") {
      ...
      layout(title = "F", yaxis = list(ticklen = 5))
      subplot(bar_plot, line_plot, nrows = 2, shareX = TRUE, titleY = TRUE) %>%
  ...
  dataset_input_ibc <- reactive({
    if (input$dataset_type == "After filtering" && input$bs_reports == "GCTA inbreeding coefficient report") {
      ...
      layout(yaxis = list(title = "Count", ticklen = 5))
      ...
      layout(title = "Fhat1", yaxis = list(ticklen = 5))
      ...
      layout(title = "Fhat2", yaxis = list(ticklen = 5))
      ...
      layout(title = "Fhat3", yaxis = list(ticklen = 5))
  ...
  ps_data_pca <- reactive({
      if (input$cultivars_pca == "ALL") {
        df_pca <- data.frame(SampleId = cultivar[,2], C1 = vals$c1vec, C2 = vals$c2vec, ClusterNo = as.character(clusters[,2]), ps = 1)
      } else if (input$cultivars_pca != "ALL") {
        Sample_id_val <- as.character(input$cultivars_pca)
        ps_vec <- c()
        for (i in seq(1, nrow(cultivar), by = 1)) {
          if (cultivar[i,2] == Sample_id_val) {
            ps_vec <- c(ps_vec, 2)
          } else {
            ps_vec <- c(ps_vec, 1)
          }
        }
        df_pca <- data.frame(SampleId = cultivar[,2], C1 = vals$c1vec, C2 = vals$c2vec, ClusterNo = as.character(clusters[,2]), ps = ps_vec)
      }
  })
  ...
  dataset_input_pca <- reactive({
    ...
    layout(paper_bgcolor = "#FFE4E1", plot_bgcolor = "#FFFFFF", title = list(text = vals$title_str, font = list(family = "sans serif", size = 20, color = "#C71585")),
      ...
      yaxis = list(title = vals_pca_pl$ytitle, range = c(vals2$ymin, vals2$ymax), showgrid = TRUE, gridwith = 1, zerolinewidth = 1, ticklen = 5), showlegend = TRUE) %>%
    config(toImageButtonOptions = list(format = u_parameters[1,19], filename = "psa_chart", width = 1000, height = 800))
  })
  ...
  output$save_text_fst <- downloadHandler(
    ...
    content = function(file) {
      ...
      if ((u_parameters[1,21] == "var_no") || (u_parameters[1,21] == "chr_no_var_no")) {
        file_name <- ""
        for (i in seq(1, nrow(fst_vals), by = 1)) {
          new_cl_pair <- ""
          new_cl_pair <- sprintf("%s-%s", fst_vals[i,1], fst_vals[i,2])
  ...
  ksh_lab_row <- reactive({
    if (input$sort_order == "PLINK Sample ID") {
      if (input$data_type_ksh == "IBS matrix") {
        lab_row_text <- rownames(ibs_matrix_os)
      } else if (input$data_type_ksh == "Relationship matrix") {
        lab_row_text <- rownames(rel_matrix_os)
      } else if (input$data_type_ksh == "KING-robust kinship") {
        lab_row_text <- rownames(king_matrix_os)
      }
    } else if (input$sort_order == "Group/Cluster number") {
      if (input$data_type_ksh == "IBS matrix") {
        lab_row_text <- rownames(ibs_matrix)
      } else if (input$data_type_ksh == "Relationship matrix") {
        lab_row_text <- rownames(rel_matrix)
      } else if (input$data_type_ksh == "KING-robust kinship") {
        lab_row_text <- rownames(king_matrix)
      }
    }
  })
  ...
  dataset_input_ksh <- reactive({
    ...
    config(toImageButtonOptions = list(format = u_parameters[1,19], filename = "ksh_chart", width = 1000, height = 800))
  })
