...
- Install the required R packages in Linux-based operating systems if you want to run the Shiny application on the [Linux Shiny Server](https://github.com/rstudio/shiny-server), or in [RStudio](https://www.rstudio.com/products/rstudio) if you want to run the Shiny application in the RStudio Desktop. Installing the Shiny package and the Shiny Server on a Linux OS normally requires root privileges. RStudio Desktop is a regular desktop application running on Windows, MacOS, or Linux that can be downloaded from the ['Download RStudio Desktop'](https://rstudio.com/products/rstudio/download/) website. Installation of RStudio Desktop is easier if you have root (or administrator privileges), but you can also do it without these privileges.
