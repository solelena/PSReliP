...
source /lustre/home/elena/PRJEB1254_fastq_files/plink_processing/all_data/art_used/malawi_cichlids_type_8gr_ld.config

echo "VCFTOOLS_HOME=$VCFTOOLS_HOME" >&2
...
PERL_PROGRAM_HOME=${TOOL_INSTALL_DIR}/program_files/perl_program
SHINY_PROGRAM_HOME=${TOOL_INSTALL_DIR}/program_files/shiny_program
...
if [ ${SAM_SELECT_FLAG} -eq 1 ]
then
  "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --keep "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_samples.list --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --sample-counts --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_sc
  ...
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --missing sample-only --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_miss
  ...
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --make-just-fam --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv_b
else
  "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --sample-counts --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_sc
  perl "${PERL_PROGRAM_HOME}"/ps_columns_names_edit.pl "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_sc.scount "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/plink_sample_counts.scount
  "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --missing sample-only --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_miss
  perl "${PERL_PROGRAM_HOME}"/ps_columns_names_edit.pl "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_miss.smiss "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/plink_missing.smiss
  SNP_NO_ALL=$( grep -vc '^#' "${BED_INPUT_FILE}.pvar" )

  "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --make-just-fam --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv_b
fi
...
then
  ...
  then
    ...
    then
      "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --snps-only --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    else
      "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --chr ${RANGE_OF_CHROMOSOMES} --snps-only --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    fi
  else
    if [ "${EXTRA_CHR_FLAG}" -eq 1 ]
    then
      "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    else
      "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_sv --allow-extra-chr --chr ${RANGE_OF_CHROMOSOMES} --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    fi 
  fi
else
  if [ "${SNP_ONLY_FLAG}" -eq 1 ]
  then
    if [ "${EXTRA_CHR_FLAG}" -eq 1 ]
    then
      "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --snps-only --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    else
      "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --chr ${RANGE_OF_CHROMOSOMES} --snps-only --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    fi
  else
    if [ "${EXTRA_CHR_FLAG}" -eq 1 ]
    then
      "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    else
      "${PLINK2_HOME}"/plink2 --pfile "${BED_INPUT_FILE}" --allow-extra-chr --chr ${RANGE_OF_CHROMOSOMES} --mind "$MIND_VAL" --geno "$GENO_VAL" --maf "$MAF_VAL" --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b
    fi
  fi
fi
 
"${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --sample-counts --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_af_sc
...
"${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --missing sample-only --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_af_miss
...
"${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --make-bed --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl_b
...
then
  ...
  then
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --indep-pairwise "$LD_WINDOW_SIZE" "$LD_WINDOW_SIZE_UNITS" 1 "$LD_THRESHOLD" --memory "$MAX_MEM_USAGE" --out "${PRUNED_SET_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"
  else
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --indep-pairwise "$LD_WINDOW_SIZE" "$LD_STEP_SIZE" "$LD_THRESHOLD" --memory "$MAX_MEM_USAGE" --out "${PRUNED_SET_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"
  fi
  ...
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --extract "${PRUNED_SET_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}".prune.in --make-pgen --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --extract "${PRUNED_SET_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}".prune.in --make-bed --memory "$MAX_MEM_USAGE" --out "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl_b
  ...
  "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --het 'cols=+het,+het' --memory "$MAX_MEM_USAGE" --out "${BS_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_het
  ...
  then
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --fst CATEGORY 'report-variants' --pheno "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups.list --memory "$MAX_MEM_USAGE" --out "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_fst
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --fst CATEGORY --pheno "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups.list --memory "$MAX_MEM_USAGE" --out "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_fst
    perl "${PERL_PROGRAM_HOME}"/ps_columns_names_edit.pl "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_fst.fst.summary "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/plink_fst.fst.summary
    perl "${PERL_PROGRAM_HOME}"/ps_fst_summary_edit.pl "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_fst.fst.summary "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups_names_numbers.list "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/results_data.fst.summary
  fi

  "${PLINK_HOME}"/plink --bfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl_b --allow-extra-chr --distance square ibs --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl_ld"${LD_THRESHOLD}"_ibs
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --make-king square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"_king
  ...
  then
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --pca meanimpute --memory "$MAX_MEM_USAGE" --out "${PSA_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"_im_pca
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --make-rel meanimpute square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"_grm
    ...
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --pca --memory "$MAX_MEM_USAGE" --out "${PSA_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"_pca
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_ld"${LD_THRESHOLD}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --make-rel square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_ld"${LD_THRESHOLD}"_grm
  ...
  then
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --fst CATEGORY 'report-variants' --pheno "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups.list --memory "$MAX_MEM_USAGE" --out "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_fst
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --fst CATEGORY --pheno "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups.list --memory "$MAX_MEM_USAGE" --out "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_fst
    perl "${PERL_PROGRAM_HOME}"/ps_columns_names_edit.pl "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_fst.fst.summary "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/plink_fst.fst.summary
    perl "${PERL_PROGRAM_HOME}"/ps_fst_summary_edit.pl "${FST_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_fst.fst.summary "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_groups_names_numbers.list "${SHINY_APP_DIR}"/"${OUTPUT_PREFIX}"/data/results_data.fst.summary
  fi

  "${PLINK_HOME}"/plink --bfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl_b --allow-extra-chr --distance square ibs --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl_ibs
  "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --make-king square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_king
  ...
  then
    ...
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --pca meanimpute --memory "$MAX_MEM_USAGE" --out "${PSA_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_im_pca
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --make-rel meanimpute square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_grm
    ...
    "${PLINK2_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --pca --memory "$MAX_MEM_USAGE" --out "${PSA_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_pca
    ...
    "${PLINK2_DEV_HOME}"/plink2 --pfile "${INPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_b --allow-extra-chr --read-freq "${ALLELE_FREQ_COUNTS_FILE}" --make-rel square --memory "$MAX_MEM_USAGE" --out "${KSH_OUTPUT_DIR}"/"${OUTPUT_PREFIX}"_pl2_grm
