...
  We prepared four Shiny apps that differ in whether they plot the Manhattan plot for FST values for each variant between pairs of subpopulations and heatmaps for relationships between pairs of samples.<br>
  ...
  - Due to the long loading time of the heatmaps if the number of samples is large and that the information in these heatmaps is difficult to see without strong zooming, we create heatmaps only if the number of samples ≤ 400. In this case, the pipeline will copy the Shiny app.R file containing the heatmaps for IBS, GRM, and KING kinship coefficient matrices from the 'with_fst_plot' or 'without_fst_plot' subfolders of the 'shiny_programs' folder to the resulting Shiny application folder. If the number of samples > 400, heatmaps are not created, but users retain the ability to download the original results of this type of analysis. In this case, the Shiny app.R file without heatmaps (from the "with_fst_plot_without_heatmap" or "without_fst_plot_without_heatmap" subfolders of the "shiny_programs" folder) will be used.
### The pipeline configuration file
